# actions.py (FINAL, CLEANED VERSION)
...
# Set up logging for clarity in the console
...
def call_zoho_reschedule_api(old_id, new_datetime):
    logger.info(f"[API CALL] Rescheduling ID {old_id} to {new_datetime}...")
    if old_id == "FAIL123":
...
def call_deny_api(old_id):
    logger.info(f"[API CALL] Denying slot for ID {old_id}. Making slot available...")
    return True

def call_cancel_api(appointment_id):
    logger.info(f"[API CALL] Cancelling appointment ID {appointment_id}...")
...
class ActionListAppointmentsWithButtons(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict) -> List[Dict]:
        ...
        # Determine context based on the intent that triggered the action
        ...
        # Note: Slot Offer flow uses a different setup action before this, so we check slot
        elif tracker.get_slot("appointment_action_type") == "slot_offer":
             context = "slot_offer"
             title_text = "We found an earlier slot! Which appointment would you like to update?"
        else:
        ...
        for idx, appt in enumerate(appointments, start=1):
            msg += f"**{idx}** {appt['service']} — {appt['doctor']} — {appt['current_time']}\n"
...
class RescheduleForm(Action):
    """Placeholder required for Rasa to register the form action."""
    def name(self) -> Text:
...
class ValidateRescheduleForm(FormValidationAction):
    ...
    def validate_datetime(self, slot_value: Any, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict) -> Dict[Text, Any]:
        ...
        # 1. Handle Suggested Slot Selection
        ...
        # 2. Check for Denial/Exit intent
        latest_intent = tracker.latest_message.get("intent", {}).get("name")
        if latest_intent in ["deny", "goodbye"]:
             # The form rule 'Abandon reschedule form on deny' should handle deactivation/goodbye
             return {"datetime": None, "suggested_slots": None, "requested_slot": None}

        # 3. Check for time entity
        time_entity = next(tracker.get_latest_entity_values("time"), None)
        
        if not time_entity:
            # If the value is None and no entity was found, the form will automatically re-ask the question.
            return {"datetime": None}
            
        # 4. Validate time entity
        try:
        ...
        if not is_valid_time(dt):
            ...
            list_msg = "\n".join([f"{idx} {s}" for idx, s in enumerate(slots, start=1)])
...
class SlotOfferConfirmationForm(Action):
    """Placeholder required for Rasa to register the form action."""
    def name(self) -> Text:
        return "slot_offer_confirmation_form"
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict) -> List[Dict]:
        return []

class ActionSetSlotOfferDetails(Action): 
    """Sets the slots required for the Slot Offer Confirmation form/rule flow."""
    def name(self) -> Text:
        return "action_set_slot_offer_details"

    async def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        # Assume these slots were set by the external system before the EXTERNAL_SLOT_OFFER intent
        new_slot = tracker.get_slot("new_slot_datetime") or "2025-10-23 09:00"
        old_id = tracker.get_slot("old_appointment_id") or "A123"
        unique_offer_id = tracker.get_slot("slot_offer_id") or "SLOT456"
        ...
        # Set slots that the form/utterance will need
        events = [
            SlotSet("old_appointment_id", old_id),
            SlotSet("slot_offer_id", unique_offer_id),
            SlotSet("old_appointment_datetime", old_time),
            SlotSet("new_slot_datetime", new_slot),
            SlotSet("appointment_action_type", "slot_offer") # Sets context for appointment listing
        ]
        return events
        
class ActionConfirmSlotOffer(Action): 
    """Handles the submission of a confirmed slot offer."""
    def name(self) -> Text:
    ...
    async def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        events.extend([
            ...
            SlotSet("slot_offer_confirmation", None), SlotSet("appointment_action_type", None)
        ])
        return events


class ActionDenySlotOffer(Action): 
    """Handles the submission of a denied slot offer."""
    def name(self) -> Text:
...
class ValidateCancelForm(FormValidationAction):
    ...
    # NOTE: The only slot in the cancel_form is 'appointment_selection'.
    # Because the form is deactivated immediately after this validation, 
    # the subsequent action (action_cancel_appointment) handles the final cancellation logic.

    def validate_appointment_selection(self, slot_value: Any, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict) -> Dict[Text, Any]:
        
        appointments = tracker.get_slot("appointments_list")
        selection = str(slot_value) # Selection from payload is stored as text

        if not appointments:
            dispatcher.utter_message(text="No appointments found. Please start the cancellation process again.")
            return {"appointment_selection": None, "requested_slot": None, "appointments_list": None}

        try:
            idx = int(selection) - 1 
            if not (0 <= idx < len(appointments)):
                raise ValueError
        except (ValueError, TypeError): 
            dispatcher.utter_message(text="Invalid selection. Please try again.")
            return {"appointment_selection": None}
            
        # Success. The form is now complete and action_cancel_appointment runs next.
        return {"appointment_selection": selection}
...
class ActionCancelAppointment(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict) -> List[Dict]:
        ...
        appointments = tracker.get_slot("appointments_list")
        selection = tracker.get_slot("appointment_selection")
